...
// Create dedicated table if it doesn't exist
 $createTableQuery = "
...
// Function to populate dedicated table
function populateDedicatedTable($db, $date = null) {
    if ($date === null) {
        $date = date('Y-m-d');
    }
    
    // Remove existing records for date to avoid duplicates
    ...
    // Insert data from original summary table
    $insertQuery = "
...
// Build query to fetch data from DEDICATED table
                                        ...
                                        <tbody>
                                            <?php if (count($attendance_data) > 0): ?>
                                                <?php foreach ($attendance_data as $row): ?>
                                                    <?php
                                                    // Get instructor photo
                                                    $photo_query = "SELECT photo FROM instructor WHERE fullname = ?";
                                                    $photo_stmt = $db->prepare($photo_query);
                                                    $photo_stmt->bind_param("s", $row['instructor_name']);
                                                    $photo_stmt->execute();
                                                    ...
                                                    if ($photo_result && $photo_result->num_rows > 0) {
                                                        ...
                                                        $instructor_photo = $photo_row['photo'];
                                                    }
                                                    $photo_stmt->close();
                                                        ...
                                                        <td>
                                                            <div class="d-flex align-items-center">
                                                                <div class="instructor-photo me-2">
                                                                    <?php if (!empty($instructor_photo) && file_exists('../uploads/instructors/' . $instructor_photo)): ?>
                                                                        <img src="../uploads/instructors/<?php echo htmlspecialchars($instructor_photo); ?>" alt="<?php echo htmlspecialchars($row['instructor_name']); ?>">
                                                                    <?php else: ?>
                                                                        <i class="fas fa-user-tie"></i>
                                                                    <?php endif; ?>
                                                                </div>
                                                                <div>
                                                                    <strong><?php echo htmlspecialchars($row['instructor_name']); ?></strong>
                                                                </div>
                                                            </div>
                                                        </td>
